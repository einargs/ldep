...
(** `ltt` is the core logic that type-checking occurs within.
...
type ltt =
  | Var : var:N.name -> ltt
  | Universe : ltt
  | Abs : bnd:binder -> var:N.name -> body:ltt -> ltt
  | App : l:ltt -> r:ltt -> ltt
and binder =
  | Pi : ltt -> binder
  | Lam : binder
...
val free_variables : t:ltt -> Tot N.name_set
(** Calculate the free variables in a `ltt` binder. *)
val binder_free_variables : binder -> Tot N.name_set
...
*)
val free_in : N.name -> ltt -> Tot bool
...
*)
val find_non_capturing_name : n:N.name -> s:N.name_set -> Tot N.name (decreases (size s))
let rec find_non_capturing_name n s =
  if mem n s
    then let s': N.name_set = remove n s in
         find_non_capturing_name (N.inc_name n) s'
    else n
...
I created it bec
*)
private val replace_var : N.name -> N.name -> t:ltt -> Tot (ret:ltt{ltt_size ret = ltt_size t})
